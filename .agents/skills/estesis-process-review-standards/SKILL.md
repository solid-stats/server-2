---
name: estesis-process-review-standards
description: >
  Shared review foundation for every Estesis review skill — code review and spec review alike.
  Owns the canonical severity buckets (🔴🟠🟡🔵 — severity only, one axis), the report output
  format with continuous numbering, the APPROVE / REQUEST CHANGES / BLOCK verdict rules, scope
  establishment and scope discipline (findings stay tied to the change), the read-only default,
  the test-file rule, and the noise filter. The specific reviewers
  (estesis-backend-vc-code-review, estesis-frontend-react-vc-code-review,
  estesis-frontend-react-counterparty-code-review, estesis-backend-vc-swagger-spec-review)
  hard-require this skill and read it first; each adds only its stack-specific gate and
  conventions on top. Do NOT trigger this for an actual review — use the matching reviewer
  skill; this skill only defines the shared standard and is read by those skills.
...
# Estesis Review Standards — Shared Foundation

This skill is the single source of truth for **how an Estesis review behaves and reports** —
the parts that must look identical no matter what is being reviewed. The stack-specific
reviewers own *what* to check (Python layers, React/MobX conventions, OpenAPI patterns); this
skill owns the *result*: severity language, finding shape, numbering, verdict, and the
discipline that keeps a review trustworthy.
...
dependency violations are often visible only outside the changed lines — imports, the class
declaration, the surrounding method, the sibling files in the same slice/module. A hunk alone
...
which verification gates to run, and — for spec review — which YAML/iteration and supporting
docs to read instead of a git diff.
...
This is exactly why the old names confused people — "Architectural Violations" sounded like a
...
| 🔵 **Low** | Does not change behavior or the contract. | style, wording, import order, cosmetics, docstrings, a minor a11y nit. | optional |
...
- Spec scale `BLOCKER / HIGH / MEDIUM / LOW / NIT` → `🔴 / 🟠 / 🟡 / 🔵 / 🔵` (LOW and NIT both land in 🔵).
...
```markdown
# Review — <scope: branch / files / iteration>

**Scope:** <what was reviewed, base used>
**Gates:** <verification gates run, with pass/fail/not-run — reviewer-specific>

## Blockers 🔴
1. `file.py:line` [topic] — what is wrong — why it is critical — how to fix it — [conv: §X / pattern-slug]
2. ...

## High 🟠
3. `file.py:line` [topic] — violation — the correct pattern, with a short code example — [conv: §X]
4. ...

## Medium 🟡
5. `file.py:line` [topic] — problem — correct version — [conv: §X]

## Low 🔵
6. `file.py:line` [topic] — problem — correct version
...
section (`§4`), a pattern slug (`response-pagination-envelope`), an ESLint/TSConfig rule — append
it to the finding as evidence. Use it whenever there is something concrete to point at; omit it
(or use `—`) when there is not. This is what ties a finding to an objective rule rather than
taste.
...
report with its own header — a scope line, a gates summary, or a gate-result block (e.g. the
backend reviewer's Swagger Spec gate).
...
- Any 🔴 finding → **BLOCK**.
- A reviewer's own hard gate failing (e.g. a missing spec without justification) → **BLOCK**.
- Only 🟠 findings → **REQUEST CHANGES** (must fix before merge).
- Only 🟡 / 🔵 findings → **REQUEST CHANGES** (may negotiate a follow-up PR/MR).
- No findings → **APPROVE**.
