---
name: estesis-frontend-react-unit-tests
description: >
  Use when writing, reviewing, or refactoring unit tests to keep them readable, isolated,
  thorough, explicit, and aligned with TDD. Triggers: "write tests", "unit tests",
  "add test coverage", "refactor tests", "TDD", "напиши тесты", "юнит-тесты",
  "покрой тестами", "добавь тесты".
---
...
Once a suite no longer fits naturally in one test file, move the unit into a
same-named directory and use `func/func.ts` plus `func/tests/*` as the canonical
layout.

Use decomposed layout when:

- You need multiple `*.test.ts` files.
- You need a `*.test.ts` file plus test-only helpers such as `*.testUtilities.ts`.
- A `max-lines` rule is signaling that the suite needs structural simplification.

Rules for decomposed suites:

- Move the source file into a same-named folder: `func.ts` becomes
  `func/func.ts`.
- Move all test-only files under `func/tests/`, not `tests/func/`.
- Inside `tests/`, test filenames keep only scenario context: `index.test.ts`, `default.test.ts`, `invalidSource.test.ts`.
- Inside `tests/`, helper filenames also drop the unit prefix and use role-based names such as `utilities.ts` or `constants.ts`.

Migration examples:

- `getFileURL.test.ts` + `getFileURL.testUtilities.ts`
- becomes:
  - `getFileURL/getFileURL.ts`
  - `getFileURL/tests/index.test.ts`
  - `getFileURL/tests/utilities.ts`
- `getFileURL.index.test.ts`
- becomes:
  - `getFileURL/getFileURL.ts`
  - `getFileURL/tests/index.test.ts`

Repo-aligned example:

- `getBackendURL/getBackendURL.ts`
- `getBackendURL/tests/default.test.ts`
- `getBackendURL/tests/invalidSource.test.ts`
- `getBackendURL/tests/utilities.ts`

Treat `max-lines` as a refactoring signal, not a formatting game.

Preferred fixes:

- Split scenario families into separate files.
- Move helpers/data into `tests/`.
- Reduce duplication.
- Remove low-signal cases.

Forbidden fix:

- Removing whitespace or compressing AAA structure just to satisfy the line limit.
...
- In decomposed suites, helpers live in `<unit>/tests/` and use role-based names such as `utilities.ts` instead of unit-prefixed filenames.

Bad helper usage:

- Helpers that hide critical Arrange state.
- Helpers with side effects not obvious at call site.
- Keeping standalone helper files like `getFileURL.testUtilities.ts` next to the source instead of moving the suite into `<unit>/tests/`.
...
- Keeping prefixed split-suite filenames such as `getFileURL.index.test.ts` or `getFileURL.testUtilities.ts` instead of normalizing to `<unit>/tests/*`.
...
- Treating `max-lines` as a reason to squeeze whitespace or collapse AAA structure instead of simplifying the suite structurally.
